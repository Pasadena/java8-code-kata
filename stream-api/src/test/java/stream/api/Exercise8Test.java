...
public class Exercise8Test extends ClassicOnlineStore {
    ...
    public void itemsNotOnSale() {
         ...
         */
        List<String> itemListOnSale = customerStream.flatMap(customer -> customer.getWantToBuy().stream().map(item -> item.getName())).collect(Collectors.toList());

        itemListOnSale.removeAll(shopStream.flatMap(shop -> shop.getItemList().stream().map(item -> item.getName())).collect(Collectors.toList()));

        Set<String> itemSetNotOnSale = new HashSet<>(itemListOnSale);
    ...
    public void havingEnoughMoney() {
        ...
        Predicate<Customer> havingEnoughMoney = customer -> customer.getBudget() >= customer.getWantToBuy().stream().mapToInt(
                item -> onSale.stream().filter(
                        shopItem -> shopItem.getName().equals(item.getName()))
                        .sorted( (first, second) -> first.getPrice() - second.getPrice())
                        .findFirst().map(Item::getPrice).orElse(0)).sum();
        List<String> customerNameList = customerStream.filter(havingEnoughMoney).map(customer -> customer.getName()).collect(Collectors.toList());
